<template>
  <aside>
    <el-menu :default-active="this.$route.path" class="el-menu-demo" background-color="#333" text-color="#fff" active-text-color="#ffd04b" router>
      <header>
        <div class="title">天狼星</div>
        <div class="avatar-box">
          <el-image src="https://q1.qlogo.cn/g?b=qq&s=640&nk=2037177696" />
          <p>MarryDream</p>
        </div>
      </header>
      <div v-for="routes of userRoutes" :key="routes.path">
        <el-sub-menu v-if="routes.children && routes.children.length > 1" :class="{'is-active': this.$route.path === routes.path}" :index="routes.path">
